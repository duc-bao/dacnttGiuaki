...
<body>
    <div class="container">
        <h2 class = "text-center mb-5">Sửa Thể loại</h2>
        <?php
    $id = $_REQUEST["id"];//lấy id sản phẩm cần sửa từ link
    require("../model/category.php");
    $sanpham = getDataByIDCat($id);
    if($sanpham===FALSE)
        die("<h3>LỖI SQL</h3>");
    if($sanpham===NULL)
        die("<h3>KHÔNG TÌM THẤY SẢN PHẨM</h3>");
    ?>
   <form name="f1" id="f1" method="post" action="XuLySuaCat.php" enctype="multipart/form-data">
    <input type="hidden" name="id_cat" value="<?=$sanpham["id_cat"]?>">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="mb-3">
                    <label for="name_cat" class="form-label">Tên thể loại:</label>
                    <input type="text" class="form-control" id="name_cat" name="name_cat" value="<?=$sanpham["name_cat"]?>">
                </div>
            </div>
        </div>
        <div class="row">
